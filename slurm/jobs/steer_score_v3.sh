#!/usr/bin/env bash
# Score-max v3: v2's descent+milestones base + explicit Floor-1 hunting list
# (bow / chest / orc soldier / snail / fire_bow / drink_potion). Targets the
# +10pt INTERMEDIATE reservoir that v2 doesn't address (PPO-RNN 1e8 gets
# bow 66%, chest 66%, snail 61%, fire_bow 50%, orc_solider 18%, potion 30%
# — the C policy gets 2%/2%/4%/0%/0%/0%). Realistic ceiling on prompt-only
# steering of C is ~22-26 (PPO-RNN 1e8 = 27.87).

set -euo pipefail

MODE="${MODE:-achievement_max_v3}"
